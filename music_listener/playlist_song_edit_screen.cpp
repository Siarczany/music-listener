...
#include "ui_playlist_song_edit_screen.h"
...
PlaylistSongEditScreen::PlaylistSongEditScreen(QWidget *parent, QSqlDatabase *database, bool isGuest, int userId, int playlistId, int songId) :
    QWidget(parent),
    ui(new Ui::playlist_song_edit_screen)
    , database(database)
    , isGuest(isGuest)
    , userId(userId)
    ...
    , sle(new SuggestionLineEdit(this))
{
    ...
    connect(ui->cancel_pb, &QPushButton::clicked, this, showPlaylist);
...
PlaylistSongEditScreen::~PlaylistSongEditScreen()
{
    delete ui;
}

void PlaylistSongEditScreen::showPlaylist()
{
    emit toPlaylistScreen(isGuest, userId, playlistId);
}

void PlaylistSongEditScreen::setupUi()
{
    QSqlQuery query;
    query.prepare(
        "SELECT "
...
void PlaylistSongEditScreen::handleFileDialog()
{
...
void PlaylistSongEditScreen::editSong()
{
...
QStringList PlaylistSongEditScreen::artistQuery(const QString &text)
{
    QStringList list;
